...
function scatterPlot(jsonObject) {
 ...
 //******************************************************************************************************
	...
	function mouseDownEvent(d,i){	
			var ind = i;
			//kollar om det finns några selected från tableLensen och gör dom till unselected
			d3.select("#chart3")
				.selectAll("#selected")
				.attr("fill", function(d){	
			...
			//sätter den man klickat på till selected
			d3.select(this)
				.attr("id", "selected")
				.attr("class", function(d,i){	lastSelectedIndex = jsonObject.selectedIndex;
												...
												if(lastSelectedIndex != jsonObject.selectedIndex){
												    setSelectedParameters(jsonObject.selectedIndex);
														...
														//här händer något märkligt när ranking är selected
